 ...
 * CreditCard adapter
 */
class CreditCardAdapter : BaseAdapter<CreditCard>() {

  val TYPE_HEADER = 1
  val TYPE_ACTION = 2

  var actionClick: Observable<Void> = PublishSubject.create()
  var itemClick: Observable<CreditCard> = PublishSubject.create()
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
    return when (viewType) {
      ...
      else -> CreditCardViewHolder(parent)
    }
  }
  ...
  override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
    ...
    if (holder is HeaderViewHolder) {
      holder.setObject(context.getString(R.string.payment_creditcard_header))
    } else if (holder is ActionAddViewHolder) {
      holder.setObject(context.getString(R.string.payment_creditcard_action_add))
      RxView.clicks(holder.itemView).throttleFirst(1, TimeUnit.SECONDS).subscribe {
        (actionClick as PublishSubject).onNext(null)
      }
    } else if (holder is CreditCardViewHolder) {
      val creditCard = getItem(position)
      holder.setObject(creditCard)
      RxView.clicks(holder.itemView).throttleFirst(1, TimeUnit.SECONDS).subscribe {
        (itemClick as PublishSubject).onNext(creditCard)
      }
    }
  }
}
